	...
	danEffect: [
			...
			skill: {
				...
				async content(event, trigger, player) {
					...
					let count = player.getStorage("wuyu_dandao").guanxing,
						...
						cards = get.cards(7),
						cardx = (get.bottomCards(7) as any).reverse(),
		...
		},
		{
			...
			async effect(event, trigger, player) {
				...
				let result = await player
					.chooseButton([`从${player.getStorage(skill)[name] * 5}个随机技能中选择${player.getStorage(skill)[name]}个获得`, [skills, "skill"]])
		...
		},
		{
			...
			async effect(event, trigger, player) {
				...
				let result = await player
					.chooseButton([`从${player.getStorage(skill)[name] * 2}个描述中包含【${get.translation(card.name)}】的技能中选择${player.getStorage(skill)[name]}个获得`, [skills, "skill"]])
					.set("selectButton", [1, get.player().getStorage(skill)[name]])
					.forResult();
				if (result.bool) {
					await player.addSkills(result.links);
		...
		},
		{
			...
			async effect(event, trigger, player) {
				...
				let result = await player
					.chooseButton([`从${player.getStorage(skill)[name] * 2}个【${get.translation(card.name)}】的兵主技能中选择${player.getStorage(skill)[name]}个获得`, [skills, "skill"]])
					.set("selectButton", [1, get.player().getStorage(skill)[name]])
					.forResult();
				if (result.bool) {
					await player.addSkills(result.links);
		...
		},
		{
			...
			skill: {
				...
				filter(event, player) {
					...
					let num = player.getHistory("useSkill", evt => evt.skill == "wuyu_dandao_z_yinglangred" && evt.event.getParent(2).card.name == event.card.name).length,
			...
			async effect(event, trigger, player) {
				...
				game.broadcastAll(checkMod => {
					game.checkMod = function () {
						...
						if (mod != "unchanged" && ["cardEnabled", "cardEnabled2", "cardRespondable", "cardSavable"].includes(name) && player.getStorage("wuyu_dandao").z3_carduse) {
							mod = "unchanged";
						}
						if (["wuxieJudgeEnabled", "wuxieJudgeRespondable", "wuxieEnabled", "wuxieRespondable"].includes(name) && player.getStorage("wuyu_dandao").z3_carduse) {
							mod = "unchanged";
						}
						return mod;
					};
					...
					lib.hooks.checkTarget.push((target, event) => {
						...
						lib.skill.wuyu_dandao_mianyi = {
							hookTrigger: {
								block(event, player, triggername, skill) {
									...
									return event.player.getStorage("wuyu_dandao").z3_turnover && (event.player.isTurnedOver() || event.player.hasSkill("wuyu_dandao_mianyi"));
								},
							},
						};
						game.players.forEach(player => {
						...
						game.players = new Proxy(game.players, {
							get(players, value, receiver) {
								...
								let bool1 = skillEvent && skillEvent.type != "global" && skillEvent.player != dandaoPlayer && dandaoPlayer.getStorage("wuyu_dandao").z3_turnover && (dandaoPlayer.isTurnedOver() || dandaoPlayer.hasSkill("wuyu_dandao_mianyi")),
			...
			async content(event, trigger, player) {
				if (trigger.name == "phase") {
					let list = player.getStorage("wuyu_dandao_skillNoted"),
						skills = lib.config.extension_无语包_wuyu_sunhanhua_skill.filter(skill => !player.hasSkill(skill, "invisible", true, true) && skill in lib.skill);

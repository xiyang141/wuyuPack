import type { link } from "fs/promises";
...
export const wuyu_dandao: Skill = {
	...
	subSkill: {
					...
					if (bool) {
						...
						if (list.includes("type")) {
							if (!player.getStorage("wuyu_dandao").allSkill) {
								...
								list.flat().unique();
								get.info("wuyu_dandao").funcs.storage(player, "allSkill", list);
							}
							const list = player
								.getStorage("wuyu_dandao")
								.allSkill.filter(skill => !player.hasSkill(skill, "invisible", true, false))
								.randomGets(skills);
							if (list.length > 0) {
								const { bool, links } = await player
									.chooseButton({
										createDialog: [`从${skills}个随机技能中选择${skill}个获得`, [list, "skill"]],
						...
						if (list.includes("suit")) {
							...
							if (list.length > 0) {
								const { bool, links } = await player
									.chooseButton({
										createDialog: [`获得${zhanfa}个战法`, [list, "vcard"]],
						...
						if (list.includes("number")) {
							...
							const { links, bool } = await player
								.chooseButton({
									createDialog: ["视为拥有一张装备效果(不会失效且独立)", [list, "vcard"]],
								})
								.forResult();
							if (bool) {
								...
								const skills = get.info("wuyu_dandao").funcs.createEquipSkills(get.skillsFromEquips([card]));
		...
		useNo: {
			...
			init(player, skill) {
				...
				game.broadcastAll(checkMod => {
					game.checkMod = function () {
						...
						if (mod != "unchanged" && ["targetEnabled", "playerEnabled"].includes(name) && player.getStorage("wuyu_dandao")) {
							mod = "unchanged";
						}
						if ("cardUsable" == name && player.getStorage("wuyu_dandao")) {
							const num = args[2];
							if (num > mod) {
								mod = num;
							}
						}
						if (mod != "unchanged" && ["cardEnabled", "cardEnabled2", "cardRespondable", "cardSavable"].includes(name) && player.getStorage("wuyu_dandao")) {
							mod = "unchanged";
						}
						if (["wuxieJudgeEnabled", "wuxieJudgeRespondable", "wuxieEnabled", "wuxieRespondable"].includes(name) && player.getStorage("wuyu_dandao")) {
							mod = "unchanged";
						}
						return mod;
					};
			...
			async content(event, trigger, player) {
				trigger.pushHandler((event, option) => {
					if (Array.isArray(event.directHit) && event.directHit.includes(player)) {
						event.directHit.remove(player);
					}
					if (event.nowuxie || !event.card.wuxieable) {
						if (!event.directHit?.length) {
							event.directHit = [];
						}
						event.directHit.addArray(game.players.filter(target => target != player));
					}
				});
			},
		},
		...
		card: {
			...
			async content(event, trigger, player) {
				...
				trigger.cancel = function () {
					...
					if (get.info("wuyu_dandao").funcs.skill(player)) {
		...
		skill: {
			init(player, skill) {
				...
				game.broadcastAll(filterSkills => {
					game.filterSkills = (skills, player, exclude) => {
						if (player.storage.wuyu_dandao) {
							return skills;
						}
						return filterSkills(skills, player, exclude);
					};
				}, filterSkills);
			},
		},
		change: {
			...
			getIndex(event, player) {
				...
				for (const pack in lib.cardPack) {
					...
					const cards = lib.cardPack[pack];
					cards.forEach(name => {
						if (lib.card[name] && player.hasUseTarget(get.autoViewAs({ name: name }), false, false)) {
							if (name == "sha") {
				...
				event.controls = [
					ui.create.control(
						(link, btn) => {
							...
							const buttons: Button[] = Array.from(event.dialog.querySelectorAll(".button"));
							if (buttons.length > 0) {
								if (!noted[packs[start]]) {
									noted[packs[start]] = (Array.from(event.dialog.querySelectorAll(".button")) as HTMLElement[]).filter(btn => !btn.classList.contains("nodisplay"));
								}
								...
								if (!noted[packs[start]]) {
									const cards = event.getParent().indexedData[packs[start]];
									const list = [];
								...
								btn.textContent = get.translation(`${packs[start]}_card_config`);
								if (!noted[packs[start]]) {
									const cards2 = event.getParent().indexedData[packs[start]];
									const list = [];
									cards.forEach(card => {
		...
		zhongliu: {
			...
			async content(event, trigger, player) {
				const skills = player
					.getSkills("invisible", true, false)
					.filter(sk => {
						let info = get.info(sk);
						return info && !info.charlotte && get.skillInfoTranslation(sk, player).length;
					})

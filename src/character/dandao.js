  ...
  danEffect: [
    ...
    },
    {
      ...
      skill: {
        ...
        async content(event, trigger, player) {
          ...
          let count = player.getStorage("wuyu_dandao").guanxing, num = player.getRoundHistory("useSkill", (evt) => evt.skill == event.name).length, cards = get.cards(7), cardx = get.bottomCards(7).reverse(), next = player.chooseToMove();
    ...
    },
    {
      ...
      skill: {
        ...
        intro: {
          ...
          mark(dialog, storage, player) {
            let used = player.getHistory("useSkill", (evt) => evt.skill == "wuyu_dandao_qinggang").map((evt) => get.translation(evt.event.getParent(2).card.name)).join("");
        ...
        subSkill: {
          effect: {
            ...
            async content(event, trigger, player) {
              let count = player.getStorage("wuyu_dandao").qinggangCount, max = player.getStorage("wuyu_dandao").qinggang;
              let num = Math.min(5, count, max), skills = [];
              count -= num;
      ...
      async effect(event, trigger, player) {
        ...
        let num = player.getStorage(skill)[name], count = player.getStorage("wuyu_dandao").x_cuilian;
        num++;
        count -= num;
    ...
    },
    {
      ...
      skill: {
        ...
        async cost(event, trigger, player) {
          ...
          let result = await player.chooseButton(
            [
              [
                (dialog) => {
                  let num2 = get.event().dandaoArgs.num, list = get.event().dandaoArgs.list;
            ...
            num,
            list: lib.phaseName.map((phase, index) => [`${phase}${index}`, get.translation(phase.slice(0, -1))])
        ...
        async content(event, trigger, player) {
          let phaselist = event.cost_data.map((phase) => phase.slice(0, -1));
    ...
    },
    {
      ...
      skill: {
        ...
        filter(event, player) {
          ...
          return cards.filter((card) => card.hasGaintag("eternal_wuyu_dandao_copy") && get.owner(card) != player && get.position(card)).length;
        },
        async content(event, trigger, player) {
      ...
      async effect(event, trigger, player) {
        ...
        let card = cards.find((card2) => card2.name != "fugui") || cards[0];
        let cardx = game.createCard(card.name, card.suit, card.number, card.nature);
        game.broadcastAll((cardx2, card2) => cardx2.destroyed = card2.destroyed, cardx, card);
      ...
      async effect(event, trigger, player) {
        ...
        if (!player.getStorage(skill).allSkill) {
          ...
          for (let character in lib.character) {
            ...
            list.addArray(skills2);
          }
          list.flat().unique();
          get.info("wuyu_dandao").dandaoFunc.dandaoStorage(player, "allSkill", list);
        }
        let skills = player.getStorage(skill).allSkill.filter((skill2) => !player.hasSkill(skill2, "invisible", true, false)).randomGets(num * 5);
        if (!skills.length) {
          player.chat("时也,命也");
          await player.draw(3);
          return;
        }
        let result = await player.chooseButton([`从${player.getStorage(skill)[name] * 5}个随机技能中选择${player.getStorage(skill)[name]}个获得`, [skills, "skill"]]).set("selectButton", [1, num]).forResult();
        if (result.bool) {
    ...
    },
    {
      ...
      judge(cards, storage) {
        ...
        let player = get.player();
        if (player.countCards("x", (card2) => {
          let gaintags = card2.gaintag.filter((tag) => {
            let str;
        ...
        let card = cards.find((card2) => card2.name == "jinnao");
      ...
      async effect(event, trigger, player) {
        ...
        let num = player.getStorage(skill)[name];
        await player.draw(num);
        let gaintags = player.getCards("x").filter((card) => {
          let gaintags2 = card.gaintag.filter((tag) => {
            let str2;
            if (tag.startsWith("eternal_")) {
              str2 = lib.translate[tag.slice(8)];
              return str2.length && str2 != "invisible";
            }
            str2 = lib.translate[tag];
            return str2.length && str2 != "invisible";
          });
          if (gaintags2.length < 1) {
            return false;
          }
          return true;
        }).map((card) => card.gaintag).flat().unique().map((tag) => {
          if (tag.startsWith("eternal_")) {
        ...
        let { cards: cardx } = await player.chooseCard("hs", `将${num}张牌置于武将牌上${get.translation(str)}区域`, [1, num]).set("dandao_skill", cards).set("filterCard", (card) => {
          let event2 = get.event();
          return !card.gaintag.includes(event2.dandaotag) && !event2.dandao_skill.includes(card);
        }).set("dandaotag", result.links[0]).forResult();
        if (cardx.length) {
    ...
    },
    {
      ...
      async effect(event, trigger, player) {
        ...
        let num = player.getStorage(skill)[name];
        await player.draw(num);
        let gaintags = player.getCards("hs").concat(cards).filter((card) => {
          let gaintags2 = card.gaintag.filter((tag) => {
            let str2;
            if (tag.startsWith("eternal_")) {
              str2 = lib.translate[tag.slice(8)];
        ...
        let str = result.links[0].startsWith("eternal_") ? get.translation(result.links[0].slice(8)) : get.translation(result.links[0]);
        let { cards: cardx } = await player.chooseCard("hs", `令${num}张牌获得${get.translation(str)}标记`, [1, num]).set("dandao_skill", cards).set("filterCard", (card) => {
          let event2 = get.event();
          return !card.gaintag.includes(event2.dandaotag) && !event2.dandao_skill.includes(card);
        }).set("dandaotag", result.links[0]).forResult();
        if (cardx.length) {
    ...
    },
    {
      ...
      skill: {
        ...
        intro: {
          ...
          mark(dialog, storage, player) {
            ...
            dialog.addText(
              Object.keys(info.max).map((subtype) => `${get.translation(subtype)}:${info.max[subtype]}`).join("")
            );
        ...
        mod: {
          globalFrom(from, to, distance) {
            return distance + from.getStorage("wuyu_dandao").z_shenbing.name.reduce((sum, name) => sum + (lib.card[name]?.distance?.globalFrom || 0), 0);
          },
          globalTo(from, to, distance) {
            return distance + to.getStorage("wuyu_dandao").z_shenbing.name.reduce((sum, name) => sum + (lib.card[name]?.distance?.globalTo || 0), 0);
          },
          attackRange(from, distance) {
            return distance - from.getStorage("wuyu_dandao").z_shenbing.name.reduce((sum, name) => sum + (lib.card[name]?.distance?.attackFrom || 0), 0);
          },
          attackTo(from, to, distance) {
            return distance + to.getStorage("wuyu_dandao").z_shenbing.name.reduce((sum, name) => sum + (lib.card[name]?.distance?.attackTo || 0), 0);
          }
        }
      },
      cl: [1, 1],
      ...
      judge(cards, storage) {
        let card = cards.find((card2) => card2.name != "shenbing");
        return !storage.name.includes(card.name);
      },
      async effect(event, trigger, player) {
        let { skill, name, cards } = event.dandaoArgs;
        let skillx = `${skill}_${name}`;
        let card = cards.find((card2) => card2.name != "shenbing"), storage = player.getStorage(skill)[name];
        let list = storage.name.reduce((list2, cardx) => {
    ...
    },
    {
      ...
      async effect(event, trigger, player) {
        let type = ["equip1", "equip2", "equip3", "equip4", "equip5"].randomGet(), storage = player.getStorage("wuyu_dandao").z_shenbing;
        storage.max[type]++;
    ...
    },
    {
      ...
      async effect(event, trigger, player) {
        let { skill, name } = event.dandaoArgs, list = [], dialog, func = (player2, list2, skill2, name2, dialog2) => {
          dialog2 = ui.create.dialog(`选择${player2.getStorage(skill2)[name2]}张牌创造并获得,这些牌进入弃牌堆时销毁`);
          let packButtons = ui.create.div();
          for (let pack in lib.cardPack) {
            if (pack == "zhanfa") {
              continue;
            }
            let packButton2 = ui.create.button([pack, get.translation(`${pack}_card_config`)], "tdnodes");
            packButton2.classList.add("filterButton");
            packButton2.listen((event2) => {
              dialog2.querySelectorAll("div.filterButton").forEach((button) => {
                button.classList.remove("glow");
              });
              packButton2.classList.add("glow");
              dialog2.querySelectorAll("div.card.button").forEach((card) => {
            ...
            packButtons.appendChild(packButton2);
            if (lib.cardPack[pack].includes("sha")) {
              let natureList = [...lib.nature.keys()];
              list2.addArray(natureList.map((nature) => ["", "", "sha", nature]));
            }
            list2.addArray(lib.cardPack[pack].map((card) => ["", "", card]));
          }
          let packButton = ui.create.button(["clear", "清除选择"], "tdnodes");
          packButton.classList.add("filterButton");
          packButton.listen((event2) => {
          ...
          dialog2.add(packButtons);
          let createButton = (item, type, position, noclick, node) => {
            node = ui.create.buttonPresets.vcard(item, type, position, noclick);
            let counterNode = ui.create.caption("<span style=font-size:24px; font-family:xinwei; text-shadow:#FFF 0 0 4px, #FFF 0 0 4px, rgba(74,29,1,1) 0 0 3px;>×0</span>", node);
          ...
          dialog2.add([list2, createButton]);
        };
        if (event.isMine()) {
        ...
        let result = await player.chooseButton().set("dialog", dialog).set("selectButton", [1, get.player().getStorage(skill)[name]]).set("numx", get.player().getStorage(skill)[name]).set("custom", {
          add: {
            confirm(bool) {
              if (bool != true) {
                return;
            ...
            button() {
              ...
              if (event2.dialog && event2.dialog.buttons) {
                for (let i = 0; i < event2.dialog.buttons.length; i++) {
                  let button = event2.dialog.buttons[i];
          ...
          replace: {
            button(button) {
              ...
              if (ui.selected.buttons.length >= event2.numx) {
                return false;
              }
        ...
        if (result.bool) {
          ...
          let storage = player.getStorage(skill).wangmeinames, list2 = names.filter((name2) => !lib.inpile.includes(name2));
          if (list2.length) {
            storage.addArray(list2);
          ...
          let cards = result.links.map((link) => {
            ...
            game.broadcastAll((card2) => card2.destroyed = "discardPile", card);
            return card;
          });
          await player.gain(cards, "gain2");
    ...
    },
    {
      ...
      async effect(event, trigger, player) {
        let { skill, name, cards } = event.dandaoArgs;
        let card = cards.find((card2) => card2.name != "wutian") || cards[0];
        ...
        let result = await player.chooseButton([`从${player.getStorage(skill)[name] * 2}个描述中包含【${get.translation(card.name)}】的技能中选择${player.getStorage(skill)[name]}个获得`, [skills, "skill"]]).set("selectButton", [1, get.player().getStorage(skill)[name]]).forResult();
        if (result.bool) {
          await player.addSkills(result.links);
        }
      }
    },
    {
      ...
      async effect(event, trigger, player) {
        let { skill, name, cards } = event.dandaoArgs;
        let card = cards.find((card2) => card2.name != "wutian");
        let skills = get.info(skill).dandaoSkill(player, skill, name, card);
        if (!skills.length) {
          player.chat("时也,命也");
          await player.draw();
        ...
        let result = await player.chooseButton([`从${player.getStorage(skill)[name] * 2}个【${get.translation(card.name)}】的兵主技能中选择${player.getStorage(skill)[name]}个获得`, [skills, "skill"]]).set("selectButton", [1, get.player().getStorage(skill)[name]]).forResult();
        if (result.bool) {
          await player.addSkills(result.links);
        }
      }
    },
    {
      ...
      async effect(event, trigger, player) {
        let { skill, name } = event.dandaoArgs;
        let num = player.getStorage(skill)[name];
        let list = lib.zhanfa.getList().filter((zhanfa) => !player.hasZhanfa(zhanfa)).randomGets(num * 4);
        if (!list.length) {
          player.chat("时也,命也");
          await player.draw(2);
          return;
        }
        let result = await player.chooseButton([`获得${num * 2}个战法`, [list, "vcard"]]).set("selectButton", [1, num * 2]).forResult();
    ...
    },
    {
      ...
      fenjue: [["yinglang"]],
      judge(cards, storage) {
        let card = cards.find((card2) => card2.name == "yinglang");
        if (storage.includes(card.name) || cards.length < 2) {
          return false;
        }
        return cards.some((card2) => card2.name == "yinglang" && get.color(card2, false) != "red");
      },
      cl: [1, 1],
      decs: "有角色使用记录牌时,摸x张牌",
      intro(storage, use) {
      ...
      async effect(event, trigger, player) {
        let { skill, name, cards } = event.dandaoArgs, card = cards.find((card2) => card2.name != "yinglang") || cards[0];
    ...
    },
    {
      ...
      judge(cards, storage) {
        ...
        return cards.some((card2) => card2.name == "yinglang" && get.color(card2, false) != "black");
      },
      cl: [1, 1],
...
已记录牌名:${get.translation(storage.name).replace(/、/g, ",")}`;
      ...
      async effect(event, trigger, player) {
        let { skill, name, cards } = event.dandaoArgs;
        let card = cards.find((card2) => card2.name != "yinglang") || cards[0];
        let cardname = card.name, add = player.getStorage(skill).z3_addnum;
    ...
    },
    {
      ...
      skill: {
        ...
        async content(event, trigger, player) {
          let num = player.getStorage("wuyu_dandao").shashan, evt = trigger.getParent();
      ...
      async effect(event, trigger, player) {
        ...
        game.broadcastAll((checkMod2) => {
          game.checkMod = function() {
            ...
            let name = args[args.length - 2], player2 = args[args.length - 1];
            ...
            if (mod != "unchanged" && "cardname" == name && player2.getStorage("wuyu_dandao").z3_carduse) {
              let tempname = player2.getStorage("wuyu_dandao").z3_wangmei?.[args[0].cardid];
              if (tempname) {
                mod = tempname;
              } else {
                mod = args[2];
            ...
            if ("cardUsable" == name && player2.getStorage("wuyu_dandao").z3_carduse) {
              mod = Infinity;
            }
            if (mod != "unchanged" && ["cardEnabled", "cardEnabled2", "cardRespondable", "cardSavable"].includes(name) && player2.getStorage("wuyu_dandao").z3_carduse) {
              mod = "unchanged";
            }
            if (["wuxieJudgeEnabled", "wuxieJudgeRespondable", "wuxieEnabled", "wuxieRespondable"].includes(name) && player2.getStorage("wuyu_dandao").z3_carduse) {
              mod = "unchanged";
            }
            return mod;
          };
      ...
      async effect(event, trigger, player) {
        game.broadcastAll((dandaoPlayer) => {
          lib.skill.wuyu_dandao_hook = {
            player: dandaoPlayer,
            hookTrigger: {
              block(event2, player2, triggername, skill) {
                if (!player2.hasSkill(skill, "invisible", true, false)) {
                  return false;
                }
                if (triggername[-1] <= "9" && triggername[-1] >= "0") {
                  triggername = triggername.slice(0, -1);
                }
                return get.info("wuyu_dandao_hook").player.getRoundHistory("useSkill", (evt) => evt.event?.triggername.includes(triggername)).length;
              }
            }
          };
          game.players.forEach((player2) => {
            if (player2 != dandaoPlayer) {
              if (!player2._hookTrigger) {
                player2._hookTrigger = [];
              }
              player2._hookTrigger.push("wuyu_dandao_hook");
            }
          });
        }, player);
      }
      ...
      skill: {
        ...
        async content(event, trigger, player) {
          ...
          trigger.cancel = function() {
            ...
            if (get.info("wuyu_dandao").dandaoFunc.dandaoSkill(player2)) {
    ...
    },
    {
      ...
      intro(storage, use) {
        let cardname = get.translation(storage.name).replace(/、/g, ","), number = get.translation(storage.number).replace(/、/g, ","), suit = get.translation(storage.suit).replace(/、/g, ","), type = get.translation(storage.type2).replace(/、/g, ",");
      ...
      async effect(event, trigger, player) {
        let { skill, name, cards } = event.dandaoArgs, card = cards.find((card2) => card2.name != "leigong") || cards[0];
        let storage = player.getStorage(skill)[name], num = player.getStorage(skill).x_cuilian;
        ["name", "number", "suit", "type2"].forEach((name2, i) => {
          if (num >= i + 1) {
            storage[name2].add(get[name2](card, false));
          }
        });
        get.info("wuyu_dandao").dandaoFunc.dandaoStorage(player, name, storage);
      }
    },
    {
      ...
      skill: {
        ...
        filter(event, player) {
          ...
          return game.hasPlayer((curr) => {
            ...
            return !["name", "number", "suit", "type2"].some((name) => player.getStorage("wuyu_dandao_z3_luojing_ban_" + name).includes(get[name](event.card, false)));
          });
        },
        async content(event, trigger, player) {
          let list = {}, targets = game.filterPlayer((curr) => curr != player), info = player.getStorage("wuyu_dandao").z3_luojing;
          ...
          ban: {
            ...
            intro: {
              ...
              mark(dialog, storage, player) {
                ...
                let name = get.translation(info.name).replace(/、/g, ","), number = get.translation(info.number).replace(/、/g, ","), suit = get.translation(info.suit).replace(/、/g, ","), type = get.translation(info.type2).replace(/、/g, ",");
            ...
            mod: {
              cardEnabled(card, player) {
                return !["name", "number", "suit", "type2"].some((name) => player.getStorage("wuyu_dandao_z3_luojing_ban")[name]?.includes(get[name](card, player)));
              },
      ...
      async effect(event, trigger, player) {
        let { skill, name, cards } = event.dandaoArgs, card = cards.find((card2) => card2.name != "luojing") || cards[0];
        let storage = player.getStorage(skill)[name], num = player.getStorage(skill).x_cuilian, add = player.getStorage(skill).z3_addnum;
        ["name", "number", "suit", "type2"].forEach((name2, i) => {
          if (num >= i + 1) {
            storage[name2].add(get[name2](card, false));
          }
    ...
    },
    {
      ...
      async effect(event, trigger, player) {
        game.broadcastAll((dandaoPlayer) => {
          lib.hooks.checkTarget.push((target, event2) => {
            let skillEvent;
            if (event2.skill) {
              skillEvent = event2;
            } else if (event2.getParent("trigger").skill) {
              skillEvent = event2.getParent("trigger");
            } else if (event2.getParent("useSkill").skill) {
              skillEvent = event2.getParent("useSkill");
            }
            if (!skillEvent || skillEvent.player == target || skillEvent.type == "global" || !target.getStorage("wuyu_dandao").z3_turnover) {
              return;
            }
            ...
            lib.skill.wuyu_dandao_mianyi = {
              hookTrigger: {
                block(event3, player2, triggername, skill) {
                  if (!player2.hasSkill(skill, "invisible", true, false) || event3.player == player2 || event3._roundStart) {
                    return false;
                  }
                  return event3.player.getStorage("wuyu_dandao").z3_turnover && (event3.player.isTurnedOver() || event3.player.hasSkill("wuyu_dandao_mianyi"));
                }
              }
            };
            game.players.forEach((player2) => {
              if (player2 != dandaoPlayer) {
                ...
                player2._hookTrigger.push("wuyu_dandao_block");
              }
            });
            game.players = new Proxy(game.players, {
              get(players, value, receiver) {
                let event3 = get.event(), skillEvent2;
                if (event3.skill) {
                  skillEvent2 = event3;
                } else if (event3.getParent("trigger").skill) {
                  skillEvent2 = event3.getParent("trigger");
                } else if (event3.getParent("useSkill").skill) {
                  skillEvent2 = event3.getParent("useSkill");
                }
                let bool1 = skillEvent2 && skillEvent2.type != "global" && skillEvent2.player != dandaoPlayer && dandaoPlayer.getStorage("wuyu_dandao").z3_turnover && (dandaoPlayer.isTurnedOver() || dandaoPlayer.hasSkill("wuyu_dandao_mianyi")), bool2 = skillEvent2 && skillEvent2.player == dandaoPlayer, bool3 = event3.name == "arrangeTrigger" && event3.current.player == dandaoPlayer, bool4 = !skillEvent2 || skillEvent2.type == "global";
    ...
    },
    {
      ...
      skill: {
        ...
        mod: {
          cardname(card, player, name) {
            ...
            let vcard = get.autoViewAs(card, [card]), filter = event.filterCard, list = [], cardList = lib.inpile.concat(player.getStorage("wuyu_dandao").wangmeinames);
      ...
      async effect(event, trigger, player) {
        ...
        game.broadcastAll(
          (getName2, skill2, name2) => {
            get.name = (card, player2) => {
              let owner = get.owner(card);
              if (get.itemtype(owner) == "player" && get.position(card) == "s" && owner.getStorage(skill2)[name2]) {
                return game.checkMod(card, owner, card.name, "cardname", owner);
              }
              return getName2(card, player2);
            };
          },
          getName,
          skill,
    ...
    },
    {
      ...
      judge(cards, storage) {
        let player = get.player();
        let list = player.getStorage("wuyu_dandao_skillNote"), skills = lib.config.extension_无语包_wuyu_sunhanhua_skill ?? [];
  ...
  daoEffect(event, player, cards) {
    let cardInfo = cards.map((card) => {
      let name = card.name, type = get.type(card, null, false), color = get.color(card, false), subtype = get.subtype(card, false);
      return [name, type, color, subtype];
    }).flat(), effects = get.info(event).danEffect;
    let effs = [], list = [], count = 0, info = player.getStorage("wuyu_dandao");
    let num = info.x_cuilian;
    for (let effect of effects) {
      if (effect.cl?.[0] > num) {
        continue;
      }
      if (!effect.fenjue.some((list2) => list2.every((name) => get.numOf(list2, name) <= get.numOf(cardInfo, name)))) {
        continue;
      }
  ...
  dandaoSkill(player, skill, sub, card) {
    ...
    let name = card.name, type = get.type(card, null, false), num = player.getStorage(skill)[sub] * 2;
    ...
    if (_status.dandao_skill[name]) {
      ...
      for (let char of _status.characterlist) {
        ...
        if (type != "equip") {
          let skills = originSkills.map((skill2) => [skill2, ...Array.from(get.info(skill2).derivation || [])]).flat();
          for (let skill2 of skills) {
            if (get.skillInfoTranslation(skill2).includes(`【${get.translation(name)}】`)) {
              list.add(skill2);
            }
          }
        } else {
          let names = get.characterSurname(char).map((info) => info.join("")).concat([get.rawName(char)]);
      ...
      game.broadcastAll((name2, list2) => _status.dandao_skill[name2] = list2, name, list);
    }
    return _status.dandao_skill[name].filter((skill2) => !player.hasSkill(skill2, "invisible", true, false)).randomGets(num);
  },
  dandaoFunc: funcMap,
  async content(event, trigger, player) {
    let extra = [], num = Math.max(player.getStorage("wuyu_dandao").x_cuilian, 1), count = lib.config.extension_无语包_wuyu_sunhanhua ?? 0;
    await event.trigger("dandaoTriggering");
    if (count / 50 >= 1) {
      let num2 = Math.floor(count / 50);
      let cards2 = lib.inpile.slice().randomGets(num2).map((name) => game.createCard(name, lib.suit.randomGet(), get.rand(1, 13)));
      extra.addArray(cards2);
    }
    if (count / 100 >= 1) {
      let list2 = player.getStorage("wuyu_dandao").cardList, num2 = Math.floor(count / 100), names = [];
      for (let i = 0; i < num2; i++) {
        names.push(list2[get.rand(list2.length - 1)]);
      }
      let cards2 = names.map((card) => {
        if (!lib.card[card[0]]) {
          let cardInfo = {
            fullimage: true,
    ...
    let { links: cards } = await player.chooseButton(["选择两张牌炼丹<br>当前效果:无效果", list], 2).set("dandao_skill", "wuyu_dandao").set("custom", {
      replace: {
        button(button) {
          let event2 = get.event(), player2 = get.player();
          if (!event2.isMine()) {
            return;
          }
          if (button.classList.contains("selectable") == false) {
          ...
          let prompt = event2.dialog.querySelector(".caption");
          let effects2 = get.info(event2.dandao_skill).daoEffect(
            event2.dandao_skill,
            player2,
            ui.selected.buttons.map((button2) => button2.link)
          );
          ...
          if (player2.isOnline2()) {
            player2.send((prompt2, str2) => prompt2.innerHTML = str2, prompt, str);
    ...
    if (effects.length) {
      for (let effect of effects) {
        ...
        if (!effect.init && effect.mark) {
          ...
          get.info("wuyu_dandao").dandaoFunc.dandaoStorage(player, name, num2 + add);
        }
        if (effect.effect) {
        ...
        if (effect.cl) {
          let num2 = player.getStorage("wuyu_dandao").x_cuilian;
          num2 = Math.max(0, num2 - effect.cl[1]);
          get.info("wuyu_dandao").dandaoFunc.dandaoStorage(player, "x_cuilian", num2);
        }
      }
      player.markSkill("wuyu_dandao_mark");
    }
    ...
    mark: {
        ...
        game.broadcastAll(
          (skillList2, list2, player2) => {
            ...
            for (let effect of list2) {
              ...
              if (effect.mark) {
                player2.storage.wuyu_dandao[effect.name] = 0;
              } else if (typeof effect.init == "function") {
                effect.init(player2, effect.name);
              }
            }
          },
          skillList,
      ...
      intro: {
        ...
        mark(dialog, storage, player) {
          ...
          let info = player.getStorage("wuyu_dandao"), effects = get.info("wuyu_dandao").danEffect;
          for (let effect of effects) {
    ...
    skillNote: {
      ...
      filter(event, player) {
        if (event.name == "phase") {
          ...
          return !["global", "equip"].includes(event.type) && !list.includes(event.skill) && lib.translate[`${event.skill}_info`]?.length && event.skill != "wuyu_dandao";
        }
      },
      charlotte: true,
      marktext: "人",
      ...
      async content(event, trigger, player) {
        if (trigger.name == "phase") {
          let list = player.getStorage("wuyu_dandao_skillNoted"), skills = lib.config.extension_无语包_wuyu_sunhanhua_skill.filter((skill) => !player.hasSkill(skill, "invisible", true, true) && skill in lib.skill);
          list.push(trigger.player);
          player.setStorage("wuyu_dandao_skillNoted", list);
          let result = await player.chooseButton([`选择至多${(player.maxHp - 1) * 3}个技能获得,每选择3个减少1点体力上限`, [skills, "skill"]]).set("selectButton", [1, (get.player().maxHp - 1) * 3]).forResult();
          if (result.links?.length) {

...
const skills = {
  ...
  rongbian: {
    ...
    init(player, skill) {
      ...
      for (let char of _status.characterlist) {
        ...
        if (info.sex == "female" && char != "huamulan") {
          const skills2 = info.skills;
          if (skills2?.length) {
            skillMap[char] = skills2;
      ...
      if (!lib.card["rongbian_" + name]) {
        ...
        const card2 = {
          ...
          cardPrompt(card3, player2) {
            const skill2 = card3.storage.romgbian_skill;
            let cardPrompt = "";
            if (lib.skill[skill2].nobracket) {
              cardPrompt += '<div class="skilln">' + get.translation(skill2) + '</div><div><span style="font-family: yuanli">' + get.plainText(get.skillInfoTranslation(skill2)) + "</span></div><br><br>";
            } else {
              const translation = lib.translate[skill2 + "_ab"] || get.translation(skill2).slice(0, 2);
              cardPrompt += '<div class="skill">【' + translation + '】</div><div><span style="font-family: yuanli">' + get.plainText(get.skillInfoTranslation(skill2)) + "</span></div><br><br>";
            }
            return cardPrompt;
          },
          ai: {
            value(card3) {
              const skill2 = card3.storage.romgbian_skill;
              if (lib.skill[skill2].ai?.neg || lib.skill[skill2].ai?.combo) {
                return 0;
              }
              return get.skillInfoTranslation(skill2).length;
            }
          }
        };
    ...
    async content(event, trigger, player) {
      ...
      const chars = list.filter((char2) => skillMap[char2].some((skill2) => !player.hasSkill(skill2, null, false, true)));
      const char = chars.randomGet();
      const skill = skillMap[char].filter((skill2) => !player.hasSkill(skill2, null, false, true)).randomGet();
    ...
    subSkill: {
      effect: {
        ...
        async content(event, trigger, player) {
          ...
          const skills2 = cards.map((card) => card.storage.romgbian_skill).filter((skill) => player.hasSkill(skill, null, false, true));
          player.addAdditionalSkill("rongbian", skills2);
  ...
  twsuzhen: {
    ...
    filter(event, player) {
      ...
      return player.countCards("hes", (card2) => !get.is.damageCard(card2)) > 0;
    },
    async precontent(event, trigger, player) {
  ...
  twliuli: {
    ...
    async cost(event, trigger, player) {
      ...
      }).set("twliuli_value", useValue).forResult();
    },
    async content(event, trigger, player) {
  ...
  twguose: {
    ...
    async content(event, trigger, player) {
      ...
      get.autoViewAs({ name: "lebu", isCard: false }, links);
    },
    subSkill: {
      note: {
        trigger: {
    ...
    subSkill: {
      round: {
        ...
        async content(event, trigger, player) {
          ...
          player.setStorage("exjuebian_note", {});
          while (list.length) {
            ...
            const note2 = player.getStorage("exjuebian_note", {});
            if (!note2[id]) {
              note2[id] = [];
            }
            const noted = note2[id].map((i) => i[1]);
            const num = [0, 1, 2, 3, 4, 5].filter((i) => !noted.includes(i)).randomGet();
            note2[id].push([phase, num]);
            player.setStorage("exjuebian_note", note2);

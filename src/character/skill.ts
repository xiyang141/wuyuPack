...
export const skill: {
	...
	ol_sb_renche: {
		...
		async content(event, trigger, player) {
			...
			const results = await player
				...
				.set("ol_sb_renche_source", player)
				.forResult();
			let draw = 0;
	...
	ol_sb_xiaojie: {
		...
		async cost(event, trigger, player) {
			event.result = await player
				...
				.set("ol_sb_xiaojie_trigger", trigger)
				.forResult();
		},
		async content(event, trigger, player) {
			const evt = trigger.getParent();
		...
		async content(event, trigger, player) {
			...
			if (link == "effect") {
				...
				const { targets } = await player
					...
					.set("ol_dangmo_card", trigger.card)
					.forResult();
				if (targets?.length) {
	...
	dc_qianxin: {
		...
		async content(event, trigger, player) {
			...
			await next;
			cards.forEach(c => game.broadcastAll(c => c.gaintag.add("eternal_dc_qianxin_tag"), c));
		},
		ai: {
			order: 10,
			result: {
				...
				async content(event, trigger, player) {
					...
					for (let target of targets) {
						if (target == player && player.countCards("h") < 4) {
							...
							if (bool) {
								const target = _status.currentPhase;
								let num = target.getStorage("dc_qianxin_debuff", 0);
								num += 2;
								target.setStorage("dc_qianxin_debuff", num);
								target.addTempSkill("dc_qianxin_debuff");
							}
						} else {
							await player.draw({ num: 1 });
						}
		...
		async content(event, trigger, player) {
			...
			const { links } = await player
				.chooseButton({
					...
					filterButton(button) {
						const cards = get.event().dc_zhenxing_cards;
						const card = button.link;
						const suit = get.suit(card, false);
			...
			game.broadcastAll(cards => cards.forEach(c => c.gaintag.add("eternal_dc_qianxin_tag")), tops);

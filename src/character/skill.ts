...
export const skill: {
	...
	fuyue: {
		...
		addFuyue(cards, player) {
			...
			for (let card of cards) {
				...
				const oldTag = card.gaintag?.find(tag => tag.startsWith("fuyue_tag_"));
				if (oldTag) {
		...
		subSkill: {
			...
			effect: {
				...
				hiddenCard(player, name) {
					const names = player
						.getCards("h")
						.filter(c => c.gaintag?.some(tag => tag.startsWith("fuyue_tag_")))
						.map(c => c.gaintag.find(tag => tag.startsWith("fuyue_tag_")).slice(10));
	...
	wenlan: {
		...
		intro: {
			mark(dialog, storage, player) {
				...
				if (last) {
					...
					const tag = last[1].find(tag => tag.startsWith("fuyue_tag_"));
					if (tag) {
		...
		init(player) {
			...
			if (!player.storage.wenlan_note && list.length % 2 != 0) {
				...
				const evt = player.getAllHistory("lose", evt => evt.type == "use" && evt.getParent("useCard") == last)[0];
				let tags = [];
		...
		filter(event, player) {
			...
			const used = list.slice(0, index + 1);
			return used.length && used.length % 2 == 0;
		},
		async content(event, trigger, player) {
			const list = player.getStorage("wenlan_note");
			...
			let noGain = true;
			const tag = last[1].find(tag => tag.startsWith("fuyue_tag_"));
			const tag2 = last2[1].find(tag => tag.startsWith("fuyue_tag_"));
			if (tag && tag2 && get.is.ordinaryCard(card) && get.is.ordinaryCard(card2)) {
				...
				if (names.some(name => names2.includes(name))) {
					...
					if (cards.length) {
						...
						const gainCards = cards.filter(card => get.owner(card) == player);
						if (gainCards.length) {
							noGain = false;
			...
			if (noGain) {
				const { cards } = await player.chooseCard({
					...
					filterCard(card, player) {
						...
						if (selected) {
							const tag = selected.gaintag.find(tag => tag.startsWith("fuyue_tag_"));
							const selectable = card.gaintag?.some(tag => tag.startsWith("fuyue_tag_"));
							if (tag) {
								return selectable;
							} else {
								return !selectable;
				...
				return {
					...
					async content(event, trigger, player) {
						...
						targets.forEach(target => {
							...
							target.setStorage("jvao_source", player);
						});
					},
				};
			},
			...
			const { bool, links } = await user
				.chooseButton({
					...
					ai(button) {
						...
						const bool = player.countCards("hs", c => get.type2(c, false) == name && player.hasUseTarget(c)) > ui.selected.buttons.length;
						if (bool) {
							return 2;
						}
						return 1;
		...
		async content(event, trigger, player) {
			...
			const { links } = await player
				.chooseButton({
					selectButton: num,
					filterButton(button) {
						const target = get.event().dejiao_target;
						const card = button.link;
						return lib.filter.cardRecastable(card, target);
					},
				...
				const {
					...
					.chooseButton({
						...
						ai(button) {
							...
							if (link == "gain") {
								const cards = player.getCards("h", c => get.suit(c, player) != suit);
								if (cards.every(c => get.value(c) > 12)) {
									return 0;
								}
							} else {
								const num = get.effect(player, { name: "losehp" }, player, player);
					...
					.set("zexing_suit", get.suit(gain, player))
					.set("zexing_source", player)
					.forResult();
				if (link == "gain") {
					bool = false;
					const list = target.getCards("h");
					const { links: cards } = await player
						.chooseButton({
							selectButton: [1, 2],
							createDialog: ["选择两张与展示牌花色不同的牌获得", [list, "card"]],
							filterButton(button) {
								const suit = get.event().zexing_suit;
								const link = button.link;
								const target = get.event().zexing_target;
								return get.suit(link, target) != suit;
							},
							ai(button) {
								const link = button.link;
				...
				if (bool) {
					const { cards, targets } = await player
						.chooseCardTarget({
							...
							filterTarget(card, player, target) {
								const list = player.getStorage("zexing_used");
								return target != player && !list.includes(target);
							},
							selectTarget: 1,
							ai1(card) {
								return -get.value(card);
			...
			if (get.numOf(numbers, max) == 1 && number == max && !state) {
				...
				await player.chooseToDiscard({
					...
					ai(card) {
						...
						if (equips.includes(card) && get.equipValue(card) < 0) {
							return 1;
						}
						return -1;
					},
			...
			backup(links, player) {
				return {
					...
					async precontent(event, trigger, player) {
						...
						const cards = player.getCards("he", c => get.suit(c, false) == suit);
	...
	jianbai: {
		...
		async content(event, trigger, player) {
			...
			for (let card of cards2) {
				...
				const num = tag ? parseInt(tag.slice(skill.length)) + 1 : 1;
				const newTag = `${skill}${num}`;
		...
		subSkill: {
			eff: {
				...
				onremove(player, skill) {
					let cards = player.getCards("he", card => card.gaintag?.some(tag => tag.startsWith(skill)));
				...
				async cost(event, trigger, player) {
					event.result = await player
						.chooseCardTarget({
							...
							forced: true,
							ai1(card) {
								const tag = card.gaintag?.find(tag => tag.startsWith("jianbai_eff"));
								const value = 6 - get.value(card, player);
								if (tag) {
	...
	ylyglieren: {
		...
		filter(event, player) {
			if (player.hasSkill("ylyglieren_noted", null, false, false)) {
				return false;
			}
		...
		async content(event, trigger, player) {
			player.addTempSkill("ylyglieren_noted");
			...
			if (result.winner) {
				...
				winner.addSkill("ylyglieren_eff");
				const list = winner.getStorage("ylyglieren_eff");
				list.add(trigger.card);
				winner.setStorage("ylyglieren_eff", list);
			}
		},
		subSkill: {
			eff: {
				...
				filter(event, player) {
					const list = player.getStorage("ylyglieren_eff");
					return list.includes(event.card);
				},
				async content(event, trigger, player) {
					const list = player.getStorage("ylyglieren_eff");
					list.remove(trigger.card);
					player.setStorage("ylyglieren_eff", list);
	...
	ylygjuxiang: {
		...
		async content(event, trigger, player) {
			if (trigger.player == player) {
				...
				const list = player.getStorage("ylygjuxiang_note");
				list.add(trigger.card);
				player.setStorage("ylygjuxiang_note", list);
				player.addTempSkill("ylygjuxiang_eff");
			}
		},
		subSkill: {
			eff: {
	...
	ylygzier: {
		...
		round: 1,
		filter(event, player) {
			const list = player.getStorage("ylygzier_note");
			return list.length > 0;
		},
		async cost(event, trigger, player) {
			const phaseList = trigger.phaseList || lib.phaseName;
			const phases = phaseList.map((p, i) => [`${p}${i}0`, get.translation(p)]);
			const list = player.getStorage("ylygzier_note").map(p => [`${p}1`, get.translation(p)]);
			const { bool, links } = await player
				.chooseButton({
					selectButton: 2,
		...
		subSkill: {
			note: {
				...
				async content(event, trigger, player) {
					...
					player.setStorage("ylygzier_note", list);
				},
			},
		},
	},
	...
	dcqianxin: {
		...
		async content(event, trigger, player) {
			...
			await next;
			cards.forEach(c => game.broadcastAll(c => c.gaintag.add("eternal_dcqianxin_tag"), c));
		},
		ai: {
			order: 10,
			result: {
				...
				async content(event, trigger, player) {
					...
					for (let target of targets) {
						if (target == player && player.countCards("h") < 4) {
							...
							if (bool) {
								const target = _status.currentPhase;
								let num = target.getStorage("dcqianxin_debuff", 0);
								num += 2;
								target.setStorage("dcqianxin_debuff", num);
								target.addTempSkill("dcqianxin_debuff");
							}
						} else {
							await player.draw({ num: 1 });
						}
		...
		async content(event, trigger, player) {
			...
			const { links } = await player
				.chooseButton({
					...
					filterButton(button) {
						const cards = get.event().dczhenxing_cards;
						const card = button.link;
						const suit = get.suit(card, false);
						const suits = cards.map(c => get.suit(c, false));
						return get.numOf(suits, suit) == 1;
					},
				})
				.set("dc_zhengxing_cards", cards)
				.forResult();
			const gain = links || [];
			...
			game.broadcastAll(cards => cards.forEach(c => c.gaintag.add("eternal_dcqianxin_tag")), tops);
	...
	dcheji: {
		...
		subSkill: {
			sha: {
				...
				mod: {
					targetInRange(card, player, target) {
						if (get.name(card, false) == "sha" && target.hasMark("dcheji")) {
							return true;
						}
					},
				},
				prompt2: "你不以此法使用【杀】结算后，可视为对拥有“骑”的其他角色使用X张【杀】(X 为该角色“骑”的数量，此杀造成的伤害无伤害来源)",
				filter(event, player) {
					if (event.card.storage.dcheji) {
						return false;
					}
					return event.card.name == "sha" && game.hasPlayer(p => p != player && p.hasMark("dcheji") && player.canUse("sha", p, false, false));

				},
				async content(event, trigger, player) {
					const targets = game.filterPlayer(p => p != player && p.hasMark("dcheji") && player.canUse("sha", p, false, false));
					let count = 0;
					const note = {};
					for (let target of targets) {
						const id = target.playerid;
						note[id] = target.countMark("dcheji");
					}
					while (targets.length > 0) {
						const useTargets = targets.filter(p => note[p.playerid] > count);
						count++;
						const removeTargets = targets.filter(p => note[p.playerid] <= count);
	...
	dchuirui: {
			...
			const { links, bool } = await player.chooseButton({
				createDialog: [
					...
					[
						[
							["mark", "获得两个“骑”"],
							["move", "移动场上任意一个“骑”至任意其他角色处"],
							["sha", "移除一个你的“骑”视为使用一张【杀】"]
						],
						"textbutton"
					]
				],
				filterButton(button) {
					const link = button.link;
					if (link == "move") {
						return game.hasPlayer(p => p.hasMark("dcheji"))
					} else if (link == "sha") {
						return player.hasMark("dcheji") && player.hasUseTarget("sha");
					}
					return true;
				},
				ai(button) {
		...
		async content(event, trigger, player) {
			...
			const link = links[0];
			if (link == "mark") {
				player.addMark("dcheji", 2);
			} else if (link == "move") {
				const { targets } = await player.chooseTarget({
					prompt: "选择一个“骑”并移动至任意其他角色处",
					filterTarget(card, player, target) {
						if (ui.selected.targets.length > 0) {
							return target != player;
						}
						return target.hasMark("dcheji");
					},
					ai(target) {
						const player = get.player();
						const att = get.attitude(player, target);
					...
					selectTarget: 2,
					complexTarget: true,
				}).set("targetprompt", ["失去", "获得"]).set("filterOk", () => ui.selected.targets.length == 2).forResult();
				if (targets?.length) {
					const loser = targets[0];
					const gainer = targets[1];
					loser.removeMark("dcheji");
					gainer.addMark("dcheji", 1);
				}
			} else if (link == "sha") {
				player.removeMark("dcheji", 1);
				const card = get.autoViewAs({ name: "sha", storage: { dcheji: true } });
				await player.chooseUseTarget({
					card: card
				})
	...
	dcxiaohui: {
		...
		mod: {
			aiOrder(player, card, num) {
				if (typeof card == "object") {
					...
					if (evt?.card && get.is.damageCard(evt.card) && !evt.dcxiaohui && get.color(card, player) == "balck") {
						return num + 10;
					}
				}
			},
		...
		filter(event, player) {
			...
			if (!evt || !evt.card || evt.dcxiaohui) {
				return false;
			}
			return get.type2(evt.card, false) == "equip";
		},
		locked: false,
		async cost(event, trigger, player) {
			event.result = await player.chooseTarget({
				prompt: "你可移动一个“骑”并可移除所有其他角色的“骑”，其他角色每失去一个“骑” 便受到一点伤害",
				filterTarget(card, player, target) {
					if (ui.selected.targets.length > 0) {
						return true;
					}
		...
		async content(event, trigger, player) {
			...
			if (bool) {
				...
				await game.doAsyncInOrder(losers, async p => {
					const num = p.countMark("dcheji");
					p.clearMark("dcheji");
					await p.damage({ num: num });
				})
			}
		},
		init(player, skill) {
		...
		subSkill: {
			mark: {
				init(player, skill) {
					const evt = lib.skill.dcjianying.getLastUsed(player);
					if (evt?.card && get.color(evt.card, player) == "balck" && !evt.dcxiaohui) {
				...
				async content(event, trigger, player) {
					if (event.triggername == "useCard1") {
						if (get.type2(trigger.card, false) == "equip") {
							player.addTip(event.name, "骁贲 可连击");
						} else {
							player.removeTip(event.name);
						}
					} else if (trigger.dcxiaohui) {
						player.removeTip(event.name);
					}
				},
			},

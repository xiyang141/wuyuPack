...
export const skills: {
		...
		subSkill: {
			note: {
				...
				async content(event, trigger, player) {
					...
					player.when("changeHpAfter").step(async (event, trigger, player) => {
						player.setStorage("twliwu_note", []);
					});
				},
			},
			round: {
	...
	twsaoting: {
		...
		intro: {
			content(storage, player, skill) {
				if (!storage) {
					("你可以将一张伤害牌当【酒】使用。当你以此法使用牌指定已受伤角色为目标后，你摸一张牌。");
				}
		...
		filter(event, player) {
			...
			const card = get.autoViewAs({ name: name, twsaoting: true });
		...
		async cost(event, trigger, player) {
			...
			event.result = await player
				.choosePlayerCard({
					...
					filterButton(button) {
						const player = get.player();
						const target = get.event().twdangjiang_target;
					...
					ai(button) {
						...
						if (target == player) {
							return get.type2(card, player) == "basic" ? 6 - value : 0;
						}
				...
				.set("twdangjiang_target", target)
				.forResult();
		},
		async content(event, trigger, player) {
			const { cards } = event;
			await trigger.player.discard({ cards: cards, discarder: player });
			if (cards?.length) {
				const type = get.type2(cards[0], false);
				if (type == "basic") {
		...
		subSkill: {
			note: {
				...
				async content(event, trigger, player) {
					...
					player.when("changeHpAfter").step(async (event, trigger, player) => {
						player.setStorage("twdangjiang_note", []);
					});
				},
			},
		},
	...
	twsuzhen: {
		...
		intro: {
			content(storage, player, skill) {
				if (!storage) {
					("你可以将一张非伤害牌当【杀】使用。当你以此法使用牌指定未受伤角色为目标后，你摸一张牌。");
				}
		...
		filter(event, player) {
			...
			const card = get.autoViewAs({ name: name, storage: { twsuzhen: true } });
			const leach = event.filterCard;
			if (typeof leach == "function" && !leach(card, player, event)) {
				return false;
			}
		...
		subSkill: {
			note: {
				...
				async content(event, trigger, player) {
					...
					player.when(["changeHpAfter", "gainMaxHpAfter", "loseMaxHpAfter"]).step(async (event, trigger, player) => {
						player.setStorage("twguose_note", []);
					});
				},
			},
		},

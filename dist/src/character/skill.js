    ...
    subSkill: {
      ...
      effect: {
        ...
        async content(event, trigger, player) {
          ...
          trigger.set("ai2", (target, card, player2, player22, isLink) => {
            const name2 = card.gaintag?.find((tag) => tag.startsWith("fuyue_tag_"))?.slice(10);
            const value = get.effect(player2, card, player22, player22, isLink);
            if (name2) {
              const card2 = get.autoViewAs({ name: name2, isCard: true });
              const value2 = get.effect(player22, card2, player22, player22, isLink);
              if (value2 > value) {
                _status.fuyue_clicked = {
                  card,
                  name: name2
